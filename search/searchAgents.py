...
class CornersState:
    @classmethod
    def make_initial(cls, corners, startingPosition):
        return cls(cornerStatusDict={corner: False for corner in corners},
                   position=startingPosition)

    def __init__(self, cornerStatusDict=None, position=None):
        if cornerStatusDict is None:
            cornerStatusDict = {}
        self.touched = cornerStatusDict
...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startingState = CornersState.make_initial(self.corners,
    ...
    def isGoalState(self, state):
        """
        :type state CornersState
        """
        return all([state.touched[corner] for corner in
                    self.corners]) and state.position
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST,
            ...
            x, y = state.position
            next_touched = state.touched.copy()
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            if hitsWall:
                continue
            if any([nextx < 1, nexty < 1,
                    nextx >= self.walls.width,
                    nexty >= self.walls.height]):
                continue

            if (nextx, nexty) in self.corners:
                next_touched[(nextx, nexty)] = True
            successors.append((
                CornersState(cornerStatusDict=next_touched,
...
def cornersHeuristic(state, problem):
    ...
    untouched = [corner for corner in state.touched if
                 state.touched[corner] is False]
    if len(untouched) == 0:
        return 0

    totalManhattan = max(
        [manhattanDistance(state.position, corner) for corner in untouched]
    )
    return totalManhattan  # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    totalManhattan = max(
        [manhattanDistance(position, food) for food in foodGrid.asList()]
    )
